    ...
    if form.validate_on_submit():
        ...
        if form.driver_selected.data:
            ...
            result["driver"] = recommend_shaft_weight(
                form.gender.data, form.user_height.data, form.user_weight.data,
                distance, SHAFT_MULTIPLIER["driver"]
            )
        ...
        if form.wood5_selected.data:
            ...
            result["wood5"] = recommend_shaft_weight(
                form.gender.data, form.user_height.data, form.user_weight.data,
                distance, SHAFT_MULTIPLIER["wood5"]
            )
        ...
        if form.utility4_selected.data:
            ...
            result["utility4"] = recommend_shaft_weight(
                form.gender.data, form.user_height.data, form.user_weight.data,
                distance, SHAFT_MULTIPLIER["utility4"]
            )
        ...
        if form.iron7_selected.data:
            ...
            result["iron7"] = recommend_shaft_weight(
                form.gender.data, form.user_height.data, form.user_weight.data,
                distance, SHAFT_MULTIPLIER["iron7"]
            )

        # ===== 플렉스 판단 =====
        if form.swing_speed_unknown.data:
            if form.driver_selected.data:
                flex = recommend_flex_by_distance(
                    form.gender.data,
                    convert_distance(form.driver_distance.data, form.distance_unit.data)
                )
            else:
                flex = "-"

        elif form.swing_speed.data:
            speed = convert_speed(form.swing_speed.data, form.speed_unit.data)
